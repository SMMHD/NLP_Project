...
if st.button("پیدا کن") and user_input:
    with st.spinner("لطفا صبر کنید..."):
        ...
        # استفاده از الگوریتم MMR برای پیدا کردن متن‌های متنوع و مرتبط
        # fetch_k=20 یعنی ابتدا 20 متن را بررسی می‌کند و سپس 8 متن متنوع را انتخاب می‌کند
        retrieved_docs_mmr = db_instance.max_marginal_relevance_search(clean_input, k=8, fetch_k=20)

        # چون متد MMR مستقیماً score برنمی‌گرداند، یک فاصله پیش‌فرض 0.0 اختصاص می‌دهیم
        retrieved_docs = [(doc, 0.0) for doc in retrieved_docs_mmr]
